...
function targetAtPoint(x: number, y: number): DropTarget | null {
  ...
  for (const r of [DROP_RADIUS * 0.5, DROP_RADIUS]) {
    for (let a = 0; a < 360; a += 45) {
      ...
      const hit = targetInStack(document.elementsFromPoint(x + r * Math.cos(rad), y + r * Math.sin(rad)));
      if (hit) return hit;
    }
  }
  return null;
...
export default function DraggableSandyx({ size, className }: Props) {
  ...
  const onDrag = (e: MouseEvent | TouchEvent | PointerEvent) => {
    ...
    if (rafRef.current == null) rafRef.current = requestAnimationFrame(runHitTest);
  };
  ...
  return (
    <motion.img
      ...
      dragMomentum={false}
      dragElastic={0.16}
      dragTransition={{ power: 0.18, timeConstant: 200, bounceStiffness: 200, bounceDamping: 28 }}
