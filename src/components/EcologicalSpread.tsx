...
export default function EcologicalSpread({
  ...
  const binderLabel = hasPGA && hasCaco3 ? 'γ-PGA biofilm + CaCO₃ biocement'
    : hasCaco3 ? 'CaCO₃ biocement' : 'γ-PGA biofilm';
  ...
  const crustRGB: [number, number, number] = hasPGA && hasCaco3 ? [125, 211, 200]
    : hasCaco3 ? [222, 226, 230] : [14, 165, 233];
  ...
  const initSimulation = () => {
    ...
    ageGridRef.current = Array(size).fill(0).map(() => Array(size).fill(0));
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement, MouseEvent>) => {
    ...
    const col = Math.floor(((e.clientX - rect.left) / rect.width) * config.gridSize);
    const row = Math.floor(((e.clientY - rect.top) / rect.height) * config.gridSize);
    ...
    if (row >= 0 && row < config.gridSize && col >= 0 && col < config.gridSize) {
      ...
      for (let dr = 0; dr <= 1; dr++) for (let dc = 0; dc <= 1; dc++) {
        ...
        if (rr < config.gridSize && cc < config.gridSize && gridRef.current[rr]?.[cc] === 0) {
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        ...
        if (state === 1) {
          ...
          nextPGA[r][c] = Math.min(1.0, pga[r][c] + 0.12 * Math.max(0.2, moisture));
          ...
          const dilutionP = gen <= genDilution ? 0 : Math.min(1, (gen - genDilution) / genDilution);
          ...
          if (nextRes[r][c] > 0.15) {
            for (let dr = -1; dr <= 1; dr++) {
              for (let dc = -1; dc <= 1; dc++) {
                ...
                if (grid[nr][nc] === 0 && res[nr][nc] > 0.2 && Math.random() < effectiveSpreadProb) {
                  nextGrid[nr][nc] = 1;
                  nextAge[nr][nc] = 0;
                }
              }
  ...
  useEffect(() => {
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        ...
        if (state === 1) {
          R = isLightMode ? 5 : 16; G = isLightMode ? 150 : 185; B = isLightMode ? 105 : 129; // active colony
        } else if (state === 2) {
          R = 148; G = 163; B = 184; // just died
        } else if (state === 3) {
    ...
    dune.addColorStop(0, isLightMode ? 'rgba(255,247,224,0.35)' : 'rgba(120,110,90,0.16)');
    dune.addColorStop(0.5, 'rgba(0,0,0,0)');
    dune.addColorStop(1, isLightMode ? 'rgba(120,86,30,0.10)' : 'rgba(0,0,0,0.28)');
    ctx.fillStyle = dune;
    ctx.fillRect(0, 0, W, W);

    // Vignette for the overhead-lens feel.
    const vig = ctx.createRadialGradient(W / 2, W / 2, W * 0.28, W / 2, W / 2, W * 0.72);
    vig.addColorStop(0, 'rgba(0,0,0,0)');
    vig.addColorStop(1, isLightMode ? 'rgba(90,66,26,0.14)' : 'rgba(0,0,0,0.42)');
    ctx.fillStyle = vig;
    ctx.fillRect(0, 0, W, W);
    ...
    <div className={`grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 rounded-xl border transition-all duration-300 ${isLightMode ? 'bg-[#fdfaf3] border-amber-900/10 shadow-[0_4px_24px_rgba(139,94,26,0.06)]' : 'bg-[#06080d] border-slate-800 shadow-xl'}`} id="ecological-spread-panel">
      ...
      <div className={`lg:col-span-12 xl:col-span-4 p-5 rounded border transition-colors duration-300 ${isLightMode ? 'bg-white border-amber-900/10' : 'bg-[#0a0f18] border-slate-800/80'} font-sans`}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {isLinked ? (
              <div className={`py-2.5 px-3 rounded text-[11px] font-mono flex items-center gap-1.5 border ${
                ...
                <span>Linked to secretion — faster spread when γ-PGA yield is higher</span>
              </div>
            ) : (
              <input 
          ...
          <div>
            <label className={`text-[11px] mb-1.5 block font-mono tracking-wider uppercase ${isLightMode ? 'text-stone-701 font-bold' : 'text-slate-400'}`}>Map Resolution</label>
            <select 
        ...
        <div className={`mt-5 pt-4 border-t font-sans ${isLightMode ? 'border-amber-900/10' : 'border-slate-800'}`}>
          <h4 className={`text-[11px] font-bold uppercase tracking-wider mb-2.5 flex items-center gap-1 font-mono ${isLightMode ? 'text-indigo-800' : 'text-slate-400'}`}>
            ...
            <GlossaryTerm term="kill-switch">Genetic Containment</GlossaryTerm> Validation
          </h4>
          ...
          <div className="space-y-2 font-mono">
            ...
            <div className={`flex items-center justify-between p-2.5 rounded text-[11px] border ${isLightMode ? 'bg-amber-50/20 border-amber-900/10' : 'bg-[#06080d] border-slate-800'}`}>
              <span className={isLightMode ? 'text-stone-500 font-bold' : 'text-slate-400'}>Crust Coverage ({binderLabel}):</span>
            ...
            {killSwitchTriggered && (
              <div className={`border-2 border-dashed p-2.5 rounded text-[10px] font-bold leading-normal ${
                ...
                ✓ BIOSAFETY VERIFIED: MazE/MazF kill-switch operational — live microbes cleared while the {binderLabel} sand crust remains intact.
              </div>
            )}
          </div>
        </div>
      ...
      <div className={`lg:col-span-12 xl:col-span-8 p-5 rounded border flex flex-col justify-between font-sans transition-colors duration-300 ${
        ...
        <div>
          ...
          <p className={`text-[11px] leading-relaxed mb-4 ${isLightMode ? 'text-stone-605 font-medium' : 'text-slate-400'}`}>
            An overhead view of the treated patch. Click anywhere to inoculate a colony; watch it spread along the moisture front, lay down {binderLabel}, and then self-terminate as the <GlossaryTerm term="maze-mazf">MazE/MazF</GlossaryTerm> plasmid dilutes out — leaving crust but no live microbes.
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start justify-center">
            ...
            <div
              ref={canvasWrapRef}
              className={`relative aspect-square w-full max-w-[460px] flex-1 rounded-xl overflow-hidden shadow-2xl ring-1 ${isLightMode ? 'ring-amber-900/15 bg-[#e8dcc0]' : 'ring-slate-800 bg-[#06090f]'}`}
            >
              <canvas
                ref={canvasRef}
              ...
              <div className="absolute top-2 right-2 flex flex-col gap-1">
                <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded text-right font-mono border ${isLightMode ? 'bg-white/85 text-stone-800 border-amber-900/12' : 'bg-slate-950/70 backdrop-blur text-slate-300'}`}>
                  Generation {steps}
                </span>
              </div>
              <div className="absolute bottom-2 left-2">
                <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono border ${isLightMode ? 'bg-white/80 text-stone-600 border-amber-900/12' : 'bg-slate-950/60 backdrop-blur text-slate-400'}`}>
            ...
            <div className="w-full md:w-44 space-y-4">
              <div className={`font-mono text-xs space-y-3 p-3 border rounded ${isLightMode ? 'bg-[#fdfaf3]/55 border-amber-900/10 text-stone-604' : 'bg-[#06080d] border border-slate-800 text-slate-400'}`}>
                ...
                <div className="flex items-center gap-2">
                  <span className={`w-3 h-3 rounded inline-block border ${isLightMode ? 'bg-[#e8dcc0] border-amber-200' : 'bg-[#1a1712] border-slate-800'}`}></span>
                  <span>Desert sand (darker = damp)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`w-3 h-3 rounded inline-block ${isLightMode ? 'bg-[#059669]' : 'bg-[#10b981]'}`}></span>
                  <span>Active <i>B. subtilis</i></span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded inline-block" style={{ background: `rgb(${crustRGB[0]},${crustRGB[1]},${crustRGB[2]})` }}></span>
