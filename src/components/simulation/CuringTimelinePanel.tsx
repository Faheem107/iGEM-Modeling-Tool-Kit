...
export default function CuringTimelinePanel({
  isLightMode,
  prongs,
  contributions,
  grainDiameter,
  ...
  const timeline = useMemo(
    () =>
      curingTimeline(contributions, {
        grainDiameter,
        designWindMs: designWind,
      }),
  ...
  const active = useMemo(
    () =>
      prongs.filter((p) =>
        contributions.some((x) => x.prong === p && x.cohesion > 0),
      ),
    [prongs, contributions],
  );

  const matureData = useMemo(
    () =>
      timeline.maturation.map((pt) => ({
        hour: +pt.hour.toFixed(1),
        total: mN(pt.total),
      })),
    [timeline.maturation],
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          isLightMode={isLightMode}
          label="Withstands (fresh)"
          value={timeline.maxSurvivableWindFresh.toFixed(0)}
          unit="m/s"
          emphasize
          accent={
            meetsDesign
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <Panel
          title={
            <>
              <GlossaryTerm term="curing">Maturation</GlossaryTerm> over the
              spray protocol
            </>
          }
          icon={Timer}
        ...
        >
          <ResponsiveContainer width="100%" height={190}>
            <AreaChart
              data={matureData}
              margin={{ top: 6, right: 10, left: -14, bottom: 4 }}
            >
        ...
        >
          <ResponsiveContainer width="100%" height={190}>
            ...
            >
              ...
              />
              {Number.isFinite(timeline.reapplyMonths) &&
                timeline.reapplyMonths > 0 && (
                  <ReferenceLine
                    x={+timeline.reapplyMonths.toFixed(1)}
                    stroke={c.axis}
                    strokeDasharray="2 2"
                    label={{
                      value: "re-apply",
