...
function PathwayFlowMap({
  fluxes, has1, has2, objective, knockouts, isLightMode,
}: {
  fluxes: Record<string, number>;
  has1: boolean; has2: boolean; objective: Obj;
  knockouts: Set<string>; isLightMode: boolean;
}) {
  const edges: FlowEdge[] = [
    { id: 'GLT', rxn: 'Glucose uptake — PTS (ptsG)', from: [NODE.glc.x, NODE.glc.y], to: [NODE.G6P.x, NODE.G6P.y], color: 'spine' },
    { id: 'GLY', rxn: 'Glycolysis (pfkA)', from: [NODE.G6P.x, NODE.G6P.y], to: [NODE.PEP.x, NODE.PEP.y], color: 'spine' },
    { id: 'ZWF', rxn: 'Oxidative pentose-P pathway (zwf) → NADPH', from: [NODE.G6P.x, NODE.G6P.y], to: [NODE.R5P.x, NODE.R5P.y], color: 'ppp' },
    { id: 'PYK', rxn: 'Pyruvate kinase (pyk)', from: [NODE.PEP.x, NODE.PEP.y], to: [NODE.PYR.x, NODE.PYR.y], color: 'spine' },
    { id: 'PDH', rxn: 'Pyruvate dehydrogenase (pdhA)', from: [NODE.PYR.x, NODE.PYR.y], to: [NODE.ACCOA.x, NODE.ACCOA.y], color: 'spine' },
    { id: 'PYC', rxn: 'Pyruvate carboxylase — anaplerosis (pycA)', from: [NODE.PYR.x, NODE.PYR.y], to: [NODE.OAA.x, NODE.OAA.y], color: 'ana' },
    { id: 'CS', rxn: 'Citrate synthase (citZ): AcCoA + OAA', from: [NODE.ACCOA.x, NODE.ACCOA.y], to: [NODE.CIT.x, NODE.CIT.y], color: 'tca' },
    { id: 'ICDH', rxn: 'Isocitrate dehydrogenase (citC) → NADPH', from: [NODE.CIT.x, NODE.CIT.y], to: [NODE.AKG.x, NODE.AKG.y], color: 'tca' },
    { id: 'AKGD', rxn: '2-oxoglutarate DH → OAA, cycle closure (odhAB)', from: [NODE.AKG.x, NODE.AKG.y], to: [NODE.OAA.x, NODE.OAA.y], color: 'tca' },
    { id: 'GOGAT', rxn: 'GS–GOGAT glutamate synthesis (glnA/gltAB)', from: [NODE.AKG.x, NODE.AKG.y], to: [NODE.GLU.x, NODE.GLU.y], color: 'spine' },
    { id: 'LDH', rxn: 'Lactate overflow — NADH sink (ldh)', from: [NODE.PYR.x, NODE.PYR.y], to: [NODE.lac.x, NODE.lac.y], color: 'over' },
    { id: 'OVF', rxn: 'Acetate overflow (pta/ackA)', from: [NODE.ACCOA.x, NODE.ACCOA.y], to: [NODE.OVF.x, NODE.OVF.y], color: 'over' },
    { id: 'BIO', rxn: 'Biomass assembly — multi-precursor drain (growth)', from: [NODE.ACCOA.x, NODE.ACCOA.y], to: [NODE.BIO.x, NODE.BIO.y], color: 'bio' },
  ];
  if (has1) edges.push({ id: 'EX_PGA', rxn: 'γ-PGA synthase + secretion (capBCA)', from: [NODE.GLU.x, NODE.GLU.y], to: [NODE.PGA.x, NODE.PGA.y], color: 'pga' });
  if (has2) edges.push({ id: 'EX_CA', rxn: 'Carbonic anhydrase display (can)', from: [NODE.GLU.x, NODE.GLU.y], to: [NODE.CA.x, NODE.CA.y], color: 'ca' });

  const activeNodes = new Set<string>(['glc', 'G6P', 'R5P', 'PEP', 'PYR', 'ACCOA', 'OAA', 'CIT', 'AKG', 'GLU', 'lac', 'OVF', 'BIO']);
  ...
  const HEX: Record<ColorKey, string> = {
    spine: isLightMode ? '#6366f1' : '#818cf8',
    tca: isLightMode ? '#0284c7' : '#38bdf8',
    ppp: isLightMode ? '#8b5cf6' : '#a78bfa',
    ana: isLightMode ? '#c026d3' : '#e879f9',
    over: '#ef4444',
    bio: isLightMode ? '#059669' : '#34d399',
    pga: isLightMode ? '#d97706' : '#fbbf24',
    ca: isLightMode ? '#0d9488' : '#2dd4bf',
  };
  const muted = isLightMode ? '#c4bcae' : '#3a475c';
  const inkNode = isLightMode ? '#fdfaf3' : '#0a0f18';
  const nodeStroke = isLightMode ? '#57534e' : '#64748b';
  const nodeText = isLightMode ? '#1c1917' : '#e2e8f0';
  ...
  const geom = (from: [number, number], to: [number, number]) => {
    ...
    return {
      x1: from[0] + ux * 21, y1: from[1] + uy * 15,
      x2: to[0] - ux * 24, y2: to[1] - uy * 17,
      mx: (from[0] + to[0]) / 2, my: (from[1] + to[1]) / 2,
    };
  };
  ...
  return (
    <div className="w-full">
      ...
      <svg viewBox="0 0 340 470" className="w-full" style={{ maxHeight: 470 }} role="img"
        ...
        <defs>
          ...
          {(Object.keys(HEX) as ColorKey[]).map((k) => (
            <marker key={k} id={`fba-arrow-${k}`} viewBox="0 0 10 10" refX="7" refY="5"
              markerUnits="userSpaceOnUse" markerWidth="11" markerHeight="11" orient="auto-start-reverse">
              <path d="M0,0 L10,5 L0,10 z" fill={HEX[k]} />
            </marker>
          ))}
          <marker id="fba-arrow-muted" viewBox="0 0 10 10" refX="7" refY="5"
            markerUnits="userSpaceOnUse" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
          ...
          return (
            <g key={e.id}>
              ...
              <line
                ...
                stroke={col} strokeWidth={w} strokeLinecap="round" opacity={op}
              ...
              {!dead && norm > 0.04 && (
                <line
                  x1={g.x1} y1={g.y1} x2={g.x2} y2={g.y2}
                  stroke={isLightMode ? '#ffffff' : '#e2e8f0'} strokeWidth={Math.min(2.4, w * 0.45)}
              ...
              {flux > 0.05 && (
                <text x={g.mx + (e.color === 'spine' ? 12 : 0)} y={g.my - 2}
                  fontSize="7.5" fill={dead ? muted : col} fontWeight="700"
                  textAnchor="middle" style={{ paintOrder: 'stroke', stroke: inkNode, strokeWidth: 2.4 }}>
                  {flux.toFixed(1)}
                </text>
              )}
            </g>
        ...
        {Object.entries(NODE).map(([key, n]) => {
          ...
          const accent =
            key === 'PGA' ? HEX.pga : key === 'CA' ? HEX.ca : key === 'BIO' ? HEX.bio
            : (key === 'OVF' || key === 'lac') ? HEX.over
            : key === 'R5P' ? HEX.ppp
            : (key === 'OAA' || key === 'CIT' || key === 'AKG') ? HEX.tca
            : HEX.spine;
          return (
            <g key={key}>
              <ellipse
                cx={n.x} cy={n.y} rx={isProduct ? 27 : 22} ry={15}
                fill={inkNode}
                stroke={isObjNode ? accent : nodeStroke}
                strokeWidth={isObjNode ? 2.6 : 1.3}
                opacity={0.98}
              />
              <text x={n.x} y={n.y + 3} fontSize={isProduct ? 8.5 : 8.2} fill={nodeText}
                fontWeight={isObjNode ? 800 : 600} textAnchor="middle">
                {n.label}
              </text>
            </g>
          );
...
export default function FbaOptimizationModule({ isLightMode, prongs = [1], onUpdatePrecursorFlux }: Props) {
  ...
  const fluxDisplay = useMemo(() => {
    const base = ['EX_GLC', 'GLY', 'ZWF', 'PDH', 'PYC', 'CS', 'ICDH', 'AKGD', 'GOGAT', 'BIO', 'RESP', 'OVF', 'LDH', 'EX_BIOM'];
  ...
  const analysis = useMemo(() => {
    ...
    const fva = fluxVariability(network, objId, 0.999, ['GLY', 'ZWF', 'ICDH', 'AKGD', 'GOGAT', has1 ? 'PGS' : 'CAS', 'OVF']);
    ...
    const objAt = (g: number, o: number) =>
      parsimoniousFBA(buildCoreNetwork({ glucoseUb: g, o2Ub: o, knockouts: ko }).network, objId).objectiveValue;
    const glucoseShadow = glucoseUb > 0 ? Math.max(0, (objAt(glucoseUb + eps, o2Ub) - base) / eps) : 0;
    const o2Shadow = o2Ub > 0 ? Math.max(0, (objAt(glucoseUb, o2Ub + eps) - base) / eps) : 0;

    return {
      sol, growthMax, pgaMax, caMax, envelope, fva, status: sol.status,
      glucoseShadow, o2Shadow,
  ...
  const carbonToProduct = useMemo(() => {
    ...
    const prodC = productObj === OBJ_PGA ? 5 * analysis.product : 1 * analysis.product; // γ-PGA monomer ~5 C
    return {
  ...
  const controls = (
    <>
      <Panel title={<><GlossaryTerm term="constraint-based">Constraint</GlossaryTerm> Bounds &amp; <GlossaryTerm term="objective-function">Objective</GlossaryTerm></>} icon={Workflow} isLightMode={isLightMode}>
        <div className="space-y-4">
          <Slider isLightMode={isLightMode} accent="accent-amber-500" label="Glucose uptake bound" value={glucoseUb} min={2} max={25} step={0.5} unit="mmol/gDCW/h"
            onChange={setGlucoseUb} hint="Upper bound on EX_glc — the measured substrate uptake flux that scales the whole linear program." />
          <Slider isLightMode={isLightMode} accent="accent-cyan-500" label="O₂ uptake bound" value={o2Ub} min={0} max={40} step={1} unit="mmol/gDCW/h"
            onChange={setO2Ub} hint="Aerobiosis ceiling; low oxygen forces fermentation and acetate overflow." />
          <div>
      ...
      <Panel title={<>In-Silico <GlossaryTerm term="gene-knockout">Gene Knockouts</GlossaryTerm></>} icon={GitBranch} isLightMode={isLightMode}>
        ...
        <p className={`mt-2 text-[10px] ${isLightMode ? 'text-stone-500' : 'text-slate-500'}`}><GlossaryText>Setting lb=ub=0 forces the LP to reroute carbon; watch the flow map, the production envelope and the acetate overflow respond in lock-step.</GlossaryText></p>
      </Panel>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <StatCard isLightMode={isLightMode} label="Glucose shadow price" value={analysis.glucoseShadow.toFixed(2)} unit="Δobj/Δcap" accent={isLightMode ? 'text-amber-700' : 'text-amber-400'} sub={analysis.glucoseShadow > 0.01 ? 'feed is binding' : 'feed not limiting'} />
        <StatCard isLightMode={isLightMode} label="O₂ shadow price" value={analysis.o2Shadow.toFixed(2)} unit="Δobj/Δcap" accent={isLightMode ? 'text-cyan-700' : 'text-cyan-400'} sub={analysis.o2Shadow > 0.01 ? 'O₂ is binding' : 'O₂ not limiting'} />
      </div>
  ...
  const legendDot = (hex: string, label: string) => (
    <span className="inline-flex items-center gap-1">
      <span className="inline-block w-2.5 h-2.5 rounded-full" style={{ background: hex }} /> {label}
    </span>
  );
  ...
  return (
    <ModuleShell isLightMode={isLightMode} controls={controls}>
      <Panel
        title={<><GlossaryTerm term="flux">Live Pathway Flow Map</GlossaryTerm> — carbon routing at the optimum</>}
        icon={Network} isLightMode={isLightMode}
        right={<span className={`text-[9px] font-mono px-2 py-0.5 rounded-full border ${
          analysis.status === 'optimal'
            ? (isLightMode ? 'border-emerald-300 text-emerald-700 bg-emerald-50' : 'border-emerald-900/50 text-emerald-300 bg-emerald-950/30')
            : (isLightMode ? 'border-rose-300 text-rose-700 bg-rose-50' : 'border-rose-900/50 text-rose-300 bg-rose-950/30')
        }`}>LP: {analysis.status}</span>}
      >
        <PathwayFlowMap
          fluxes={analysis.sol.fluxes} has1={has1} has2={has2}
          objective={objective} knockouts={knockouts} isLightMode={isLightMode}
        />
        <div className={`mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[9px] ${isLightMode ? 'text-stone-500' : 'text-slate-400'}`}>
          {legendDot(isLightMode ? '#6366f1' : '#818cf8', 'glycolysis')}
          {legendDot(isLightMode ? '#0284c7' : '#38bdf8', 'TCA cycle')}
          {legendDot(isLightMode ? '#8b5cf6' : '#a78bfa', 'pentose-P (NADPH)')}
          {legendDot(isLightMode ? '#c026d3' : '#e879f9', 'anaplerosis')}
          {has1 && legendDot(isLightMode ? '#d97706' : '#fbbf24', 'γ-PGA')}
          {has2 && legendDot(isLightMode ? '#0d9488' : '#2dd4bf', 'CA enzyme')}
          {legendDot(isLightMode ? '#059669' : '#34d399', 'biomass')}
          {legendDot('#ef4444', 'overflow (acetate/lactate)')}
        </div>
        <p className={`mt-1.5 text-[10px] ${isLightMode ? 'text-stone-500' : 'text-slate-500'}`}>
          <GlossaryText>Arrow thickness and flow speed track each reaction&apos;s pFBA flux; the bold-ringed node is the current objective. Every arrow obeys the steady-state mass balance, so what leaves α-KG toward the product is carbon taken away from the TCA cycle and growth.</GlossaryText>
        </p>
      </Panel>
      ...
      <Panel title={<><GlossaryTerm term="production-envelope">Production Envelope</GlossaryTerm> — Growth ↔ {productLabel} <GlossaryTerm term="production-envelope">Pareto Front</GlossaryTerm></>} icon={Sigma} isLightMode={isLightMode}>
        ...
        <p className={`mt-1 text-[10px] flex items-center gap-1.5 ${isLightMode ? 'text-stone-500' : 'text-slate-500'}`}>
          <Beaker className="w-3 h-3" /> Marker = current objective&apos;s operating point. The downward slope is the growth that must be traded away per unit of product.
        </p>
      </Panel>

      <Panel title={<>Carbon Fate — where the fixed feed lands</>} icon={Sigma} isLightMode={isLightMode}>
        <div className="space-y-2.5">
          {([
            ['To product', carbonToProduct.product, isLightMode ? '#d97706' : '#fbbf24'],
            ['Wasted as acetate', carbonToProduct.acetate, '#ef4444'],
            ['Released as CO₂', carbonToProduct.co2, isLightMode ? '#64748b' : '#94a3b8'],
          ] as [string, number, string][]).map(([lbl, pct, hex]) => (
            <div key={lbl}>
              <div className="flex justify-between text-[10px] font-mono mb-0.5">
                <span className={isLightMode ? 'text-stone-600 font-bold' : 'text-slate-300'}>{lbl}</span>
                <span className={isLightMode ? 'text-stone-500' : 'text-slate-500'}>{pct.toFixed(0)}% of feed C</span>
              </div>
              <div className={`h-2 rounded-full overflow-hidden ${isLightMode ? 'bg-stone-200' : 'bg-slate-800'}`}>
                <div className="h-2 rounded-full" style={{ width: `${pct}%`, background: hex }} />
              </div>
            </div>
          ))}
        </div>
        <p className={`text-[10px] mt-2 ${isLightMode ? 'text-stone-500' : 'text-slate-500'}`}>
          <GlossaryText>Carbon-mole balance on the fixed glucose feed. Deleting acetate overflow visibly shifts the red bar into the product bar.</GlossaryText>
        </p>
      </Panel>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <Panel title={<><GlossaryTerm term="fva">Flux Variability</GlossaryTerm> (at optimum)</>} icon={GitBranch} isLightMode={isLightMode}>
          <div className="space-y-2.5 mt-1">
            ...
            <p className={`text-[10px] mt-2 ${isLightMode ? 'text-stone-500' : 'text-slate-500'}`}>Wide bars = flexible reactions with alternate optima; pinpoints = rigid, rate-limiting steps.</p>
          </div>
        </Panel>
      </div>

      <div className={`px-4 py-3 rounded-xl border text-[10px] font-mono leading-relaxed ${
        isLightMode ? 'bg-[#fcfaf5] border-amber-900/10 text-stone-600' : 'bg-[#06090f] border-slate-850 text-slate-400'
      }`}>
        <GlossaryText>
