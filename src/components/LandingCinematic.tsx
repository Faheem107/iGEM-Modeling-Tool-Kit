...
const ACCENT: Record<Accent, { text: string; textLight: string; ring: string; dot: string }> = {
  indigo: { text: 'text-indigo-300', textLight: 'text-indigo-600', ring: 'ring-indigo-400/30', dot: 'bg-indigo-400' },
  emerald: { text: 'text-emerald-300', textLight: 'text-emerald-600', ring: 'ring-emerald-400/30', dot: 'bg-emerald-400' },
  amber: { text: 'text-amber-300', textLight: 'text-amber-600', ring: 'ring-amber-400/30', dot: 'bg-amber-400' },
};
    ...
    <div className="relative w-full h-[68vh] min-h-[420px] max-h-[620px] overflow-hidden">
      ...
      <div className="absolute z-20 left-4 right-4 top-8 sm:left-auto sm:top-1/2 sm:-translate-y-1/2 sm:right-[6%] sm:w-[56%] sm:max-w-lg">
        <motion.div
          ...
          className={`rounded-3xl border p-6 sm:p-8 backdrop-blur-xl shadow-2xl ring-1 ${a.ring} ${
            isLightMode ? 'bg-white/80 border-white/70 text-slate-900' : 'bg-slate-950/70 border-white/10 text-slate-100'
          }`}
        >
          <h2 className={`text-xl sm:text-3xl font-black uppercase tracking-widest mb-4 ${isLightMode ? a.textLight : a.text}`}>
            {scene.title}
          </h2>
...
export default function LandingCinematic({ isLightMode }: { isLightMode: boolean }) {
  ...
  return (
    <section id="cinematic" className="relative w-full flex justify-center px-4 py-14 sm:py-24 scroll-mt-20">
      <div className="relative w-full max-w-5xl">
        <div
          className={`relative rounded-[2rem] overflow-hidden border shadow-2xl ring-1 ${a.ring} ${
        ...
        >
          <Carousel index={index} onIndexChange={setIndex}>
            ...
            />
            <CarouselIndicator className="bottom-5" classNameButton="!w-2.5 !h-2.5" />
          </Carousel>
