    ...
    <div className={`p-6 rounded-2xl border space-y-6 transition-colors duration-300 animate-fadeIn ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className={`lg:col-span-5 space-y-6 p-5 rounded-xl border transition-all duration-300 ${
          isLightMode ? 'bg-white/95 border-amber-900/10 shadow-md text-stone-800' : 'bg-[#0a0f18] border-slate-800 text-slate-200'
        }`}>
            ...
            <div className="space-y-5 px-1">
              <div>
                <div className={`flex justify-between text-[11px] mb-1.5 ${isLightMode ? 'text-stone-600' : 'text-slate-400'}`}>
                  <span className={`font-semibold ${isLightMode ? 'text-stone-800' : 'text-slate-200'}`}>Inoculum Cell Density (<GlossaryTerm term="OD600" />)</span>
                  <span className={`font-mono px-1.5 py-0.5 rounded text-[10px] border ${isLightMode ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-bold' : 'text-indigo-400 bg-indigo-950/40 border border-indigo-900/30'}`}>{labOD600.toFixed(2)} Abs</span>
                </div>
                <div className="flex items-center gap-3">
                  <input type="range" min="0.1" max="4.0" step="0.1" value={labOD600} onChange={(e) => setLabOD600(parseFloat(e.target.value))} className={`grow h-1.5 rounded accent-indigo-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input type="range" min="2" max="100" step="2" value={labInocVolume} onChange={(e) => setLabInocVolume(parseInt(e.target.value))} className={`grow h-1.5 rounded accent-indigo-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
                </div>
              </div>

              <div>
                <div className={`flex justify-between text-[11px] mb-1.5 ${isLightMode ? 'text-stone-600' : 'text-slate-400'}`}>
                  <span className={`font-semibold ${isLightMode ? 'text-stone-800' : 'text-slate-200'}`}><GlossaryTerm term="L-Glutamate" /> Substrate Feed</span>
                  <span className={`font-mono px-1.5 py-0.5 rounded text-[10px] border ${isLightMode ? 'bg-emerald-50 border-emerald-200 text-emerald-700 font-bold' : 'text-emerald-400 bg-emerald-950/40 border border-emerald-900/30'}`}>{labGlutamate.toFixed(0)} mM</span>
                </div>
                <div className="flex items-center gap-3">
                  <input type="range" min="0" max="120" step="5" value={labGlutamate} onChange={(e) => setLabGlutamate(parseInt(e.target.value))} className={`grow h-1.5 rounded accent-emerald-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
                </div>
              </div>

              <div>
                <div className={`flex justify-between text-[11px] mb-1.5 ${isLightMode ? 'text-stone-600' : 'text-slate-400'}`}>
                  <span className={`font-semibold ${isLightMode ? 'text-stone-800' : 'text-slate-200'}`}>Ca2+ Salinity Additive</span>
                  <span className={`font-mono px-1.5 py-0.5 rounded text-[10px] border ${isLightMode ? 'bg-amber-50 border-amber-200 text-amber-700 font-bold' : 'text-amber-400 bg-amber-950/40 border border-amber-900/30'}`}>{labSalinity.toFixed(1)} g/L</span>
                </div>
                <div className="flex items-center gap-3">
                  <input type="range" min="0.1" max="12.0" step="0.1" value={labSalinity} onChange={(e) => setLabSalinity(parseFloat(e.target.value))} className={`grow h-1.5 rounded accent-amber-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
                </div>
              ...
              <div>
                <div className={`flex justify-between text-[11px] mb-1.5 ${isLightMode ? 'text-stone-600' : 'text-slate-400'}`}>
                  ...
                  <span className={`font-mono px-1.5 py-0.5 rounded text-[10px] border ${isLightMode ? 'bg-rose-50 border-rose-200 text-rose-700 font-bold' : 'text-rose-400 bg-rose-950/40 border border-rose-900/30'}`}>{labTemp.toFixed(1)} °C</span>
                </div>
                <div className="flex items-center gap-3">
                  <input type="range" min="15" max="55" step="1" value={labTemp} onChange={(e) => setLabTemp(parseFloat(e.target.value))} className={`grow h-1.5 rounded accent-rose-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
                </div>
              </div>
            </div>
          </div>

          <div className={`p-4 rounded-xl border mt-6 ${isLightMode ? 'bg-[#fcfaf5] border-amber-900/10' : 'bg-[#020508] border-slate-800'}`}>
            <div className="flex items-center gap-2 mb-4">
              <Wind className={`w-4 h-4 ${isLightMode ? 'text-emerald-600' : 'text-emerald-400'}`} />
              <h3 className="font-bold text-sm uppercase tracking-wider font-mono">2. Storm Simulation</h3>
            </div>
            ...
            <div className="space-y-4 px-1">
              <div>
                ...
                <input type="range" min="0.10" max="1.50" step="0.05" value={windFriction} onChange={(e) => setWindFriction(parseFloat(e.target.value))} className={`w-full h-1.5 rounded accent-emerald-500 cursor-ew-resize ${isLightMode ? 'bg-stone-200' : ''}`} />
              </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer mt-2 group">
                ...
                />
                <span className={`text-[11px] font-bold uppercase transition-colors ${stormActive ? 'text-red-500 animate-pulse' : (isLightMode ? 'text-stone-600' : 'text-slate-400')}`}>
                  {stormActive ? '⚠ GALE FORCE EVENT ACTIVE' : 'Trigger Sudden Gale Event'}
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="flex gap-2 mt-4">
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg font-bold font-mono text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer border ${
                isLightMode ? 'bg-stone-50 hover:bg-stone-100 text-stone-700 border-stone-200' : 'bg-slate-900 hover:bg-slate-800 text-slate-350 hover:text-white border-slate-800'
              }`}
            >
            ...
            <button
              ...
              className={`py-2.5 px-4 rounded-lg font-bold font-mono text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer border transition-colors ${
                isLightMode ? 'bg-blue-50 hover:bg-blue-100 text-blue-700 border-blue-200' : 'bg-blue-950/40 hover:bg-blue-900/60 text-blue-400 border-blue-900/30'
              }`}
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-red-950/15 border border-red-900/40 rounded-xl relative overflow-hidden flex flex-col justify-between">
              <span className="text-[10px] font-mono font-bold text-red-500 uppercase tracking-wider mb-2">Control (Bare Sand)</span>
              <div className="flex justify-between items-end">
                <span className="text-3xl font-black text-red-400">{erosionUntreated}%</span>
                <span className="text-[10px] text-red-500/70 font-mono pb-1">Erosion</span>
              </div>
            </div>
            <div className="p-4 bg-emerald-950/15 border border-emerald-900/40 rounded-xl relative overflow-hidden flex flex-col justify-between">
              <span className="text-[10px] font-mono font-bold text-emerald-500 uppercase tracking-wider mb-2">Treated (Bio-stabilized)</span>
              <div className="flex justify-between items-end">
                <span className="text-3xl font-black text-emerald-400">{erosionTreated}%</span>
                <span className="text-[10px] text-emerald-500/70 font-mono pb-1">Erosion</span>
              </div>
            </div>
          </div>

          <div className={`p-1.5 rounded-xl border flex flex-col relative overflow-hidden transition-all duration-300 ${
            isLightMode ? 'bg-white border-amber-900/10 shadow-lg' : 'bg-[#020305] border-slate-800'
          }`}>
            <canvas ref={canvasRef} width={800} height={400} className="w-full h-auto bg-black rounded-lg cursor-crosshair border border-slate-900/50" />
            
            <div className={`absolute bottom-3 left-3 right-3 p-3 rounded-lg border backdrop-blur-md flex justify-between items-center ${
              isLightMode ? 'bg-white/80 border-amber-900/10 text-stone-800' : 'bg-black/60 border-slate-800 text-slate-300'
            }`}>
              <div className="flex flex-col">
                <span className="text-[10px] font-bold font-mono text-indigo-400 mb-0.5">Calculated Modulus (Gs)</span>
                <span className="text-sm font-black">{derivedShearModulus.toFixed(0)} Pa</span>
              </div>
              <div className="w-px h-8 bg-slate-700/50" />
              <div className="flex flex-col">
                <span className="text-[10px] font-bold font-mono text-emerald-400 mb-0.5">Yield Target (PGA)</span>
                <span className="text-sm font-black">{derivedPGA.toFixed(1)} g/L</span>
              </div>
              <div className="w-px h-8 bg-slate-700/50" />
              <div className="flex flex-col">
                <span className="text-[10px] font-bold font-mono text-rose-400 mb-0.5">Critical Friction (u*t)</span>
                <span className="text-sm font-black">{u_star_critical.toFixed(2)} m/s</span>
              </div>
            </div>
          </div>

          <div className={`border rounded-xl overflow-hidden mt-4 ${
            isLightMode ? 'bg-stone-50 border-amber-900/10' : 'bg-[#06090f] border-slate-850'
          }`}>
            <div className="flex border-b border-inherit">
              <button onClick={() => setExplainTab('context')} className={`flex-1 py-2 text-[10px] font-mono font-bold uppercase tracking-wider ${explainTab === 'context' ? (isLightMode ? 'bg-amber-100 text-amber-900' : 'bg-slate-800 text-white') : (isLightMode ? 'text-stone-500 hover:bg-stone-100' : 'text-slate-500 hover:bg-slate-900/50')}`}>Wet-Lab Context</button>
              <button onClick={() => setExplainTab('math')} className={`flex-1 py-2 text-[10px] font-mono font-bold uppercase tracking-wider ${explainTab === 'math' ? (isLightMode ? 'bg-amber-100 text-amber-900' : 'bg-slate-800 text-white') : (isLightMode ? 'text-stone-500 hover:bg-stone-100' : 'text-slate-500 hover:bg-slate-900/50')}`}>Bio-Physical Math Models</button>
            </div>
              ...
              {explainTab === 'context' ? (
                <div className="animate-fadeIn space-y-3 leading-relaxed">
                  <p className={isLightMode ? 'text-stone-700 font-medium' : 'text-slate-350'}>
                    Our synthetic <em className={`italic font-semibold ${isLightMode ? 'text-amber-900' : 'text-slate-100'}`}>Bacillus subtilis</em> cells secrete robust <span className="font-bold text-emerald-500">γ-PGA polymers</span>.
                  </p>
                  <p className={isLightMode ? 'text-stone-700 font-medium' : 'text-slate-400'}>
                    Here we tie <b>physical Wet Lab factors</b> directly into real-world aerodynamic thresholds:
                  </p>
                  <ul className={`list-disc list-inside space-y-1 ml-1 ${isLightMode ? 'text-stone-600' : 'text-slate-500'}`}>
                    <li><b>Higher OD600</b> guarantees dense biofilm matrices.</li>
                    <li><b>L-Glutamate Precursors</b> map directly to PGA yields.</li>
                    <li><b>Divalent Salinity</b> is critical; calcium acts as the thermodynamic "glue" crossing PGA carboxyl groups.</li>
                  </ul>
                </div>
              ) : (
                <div className="animate-fadeIn space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className={`p-2.5 border rounded ${isLightMode ? 'bg-[#fcfaf5]/85 border-[#b8956c]/20' : 'bg-[#04060a] border-slate-850'}`}>
                      <span className="text-rose-500 font-bold block mb-1">1. Temperature Viability:</span>
                      <code>Yield ∝ Feed * (1 - α|T - T_opt|)</code>
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal font-sans">
                        Metabolic productivity penalty function tracking divergence from ideal 37°C.
                      </p>
                    </div>
                    <div className={`p-2.5 border rounded ${isLightMode ? 'bg-[#fcfaf5]/85 border-[#b8956c]/20' : 'bg-[#04060a] border-slate-850'}`}>
                      <span className="text-amber-500 font-bold block mb-1">2. Salinity Saturation:</span>
                      <code>θ = S / (Kd + S)</code>
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal font-sans">
                        Michaelis-Menten coordination saturation scaling for matrix cohesiveness.
                      </p>
                    </div>
                    <div className={`p-2.5 border rounded ${isLightMode ? 'bg-[#fcfaf5]/85 border-[#b8956c]/20' : 'bg-[#04060a] border-slate-850'}`}>
