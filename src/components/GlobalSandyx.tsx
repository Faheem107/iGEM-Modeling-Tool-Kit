...
export default function GlobalSandyx({ isLanding, hideOnDesktop, isLightMode }: Props) {
  ...
  return (
    <div className={`fixed bottom-4 left-4 z-[80] ${hideOnDesktop ? 'lg:hidden' : ''}`}>
      <AnimatePresence>
        {visible && (
          <motion.div
            key="global-sandyx"
          ...
          >
            {!dragging && (
              <motion.span
                ...
                className={`mb-1 max-w-[140px] text-center text-[9.5px] font-semibold leading-tight px-2.5 py-1 rounded-2xl border shadow-sm ${
                  isLightMode
              ...
              >
                Drop me at any <span className="text-teal-500 underline decoration-dotted">underlined</span> word!
              </motion.span>
            )}
