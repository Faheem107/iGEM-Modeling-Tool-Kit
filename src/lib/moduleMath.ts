    ...
    blocks: [
      {
        ...
        caption: 'Maximise the objective flux subject to mass balance (S·v = 0) and reaction bounds.',
      },
      {
        tex: '\\min \\sum_i |v_i| \\quad \\text{s.t.}\\quad c^{\\top} v = Z^{*}',
        caption: 'Parsimonious FBA: among all optima, choose the one using the least total flux (enzyme cost).',
      },
      {
        tex: '[S] = \\kappa \\cdot v_{\\text{glu}}',
        caption: 'The precursor export flux is converted to an intracellular concentration that feeds the kinetics model.',
      },
    ],
  },
  metabolic: {
    ...
    blocks: [
      {
        ...
        caption: 'mRNA balance: constitutive transcription minus first-order decay.',
      },
      {
      ...
      },
      {
        ...
        caption: 'γ-PGA accumulation: Michaelis–Menten synthesis minus degradation (k_deg → 0 under both knockouts).',
      },
    ],
  },
  crosslink: {
    ...
    blocks: [
      ...
      },
      {
        ...
        caption: 'Effective cross-link density, with the finite-chain end correction.',
      },
      {
        tex: 'G = \\nu R T',
    ...
    blocks: [
      {
        ...
        caption: 'Overall display efficiency multiplies export, dimerisation and anchoring efficiencies.',
      },
      {
        tex: 'a_{\\mathrm{CA}} = \\frac{\\log_{10}(1 + f\\,E)}{\\log_{10}(1 + E)}',
        caption: 'Normalised CA activity (log-scaled, since enhancement E spans orders of magnitude).',
      },
    ],
  },
  caco3: {
    ...
    blocks: [
      {
        ...
        caption: 'Carbonate fraction of dissolved inorganic carbon at a given pH (Bjerrum).',
      },
      {
        tex: '\\Omega = \\frac{[\\mathrm{Ca}^{2+}][\\mathrm{CO_3^{2-}}]}{K_{sp}},\\qquad SI = \\log_{10}\\Omega',
        caption: 'Saturation ratio and index; SI > 0 means precipitation is favourable.',
      },
      {
        tex: '\\mathrm{ACC} \\xrightarrow{k_r} f_v\\,\\text{vaterite} + (1-f_v)\\,\\text{calcite},\\qquad \\text{vaterite} \\xrightarrow{k_{vc}} \\text{calcite}',
        caption: 'Non-ureolytic CA-MICP crystallises mostly metastable vaterite (fraction f_v), which slowly recrystallises to calcite (Ostwald ripening).',
      },
      {
        tex: 'w_{\\text{eff}} = w_{\\text{cal}} + s_v\\,w_{\\text{vat}},\\qquad \\mathrm{UCS} = k_{\\mathrm{UCS}}\\,w_{\\text{eff}}^{\\,n_{\\mathrm{UCS}}}',
        caption: 'Load-bearing carbonate wt%: vaterite counts at a reduced strength factor s_v until it converts; UCS is a power law of it [kPa].',
      },
      {
        tex: 'm_{\\mathrm{CO_2}} = n_{\\mathrm{CaCO_3}} \\cdot M_{\\mathrm{CO_2}}',
        caption: 'Each mole of carbonate (any polymorph) sequesters one mole of CO₂ (climate co-benefit).',
      },
    ],
  },
  alginate: {
    ...
    blocks: [
      {
        ...
        caption: 'Egg-box junction density (guluronate-weighted) and the resulting gel modulus.',
      },
      {
        tex: 'R(n) = (1 - k)^{\\,n}',
        caption: 'Residual alginate after n rain/wet cycles — its honest solubility limitation.',
      },
    ],
  },
  thermal: {
    ...
    blocks: [
      ...
      },
      {
        ...
        caption: 'Folded fraction (Boltzmann two-state); the melting point is where f = ½.',
      },
    ],
  },
  'protein-3d': {
    ...
    blocks: [
      {
        ...
        caption: 'Each residue is placed by its Cα coordinate; the ribbon interpolates the backbone.',
      },
    ],
  },
  ecological: {
    ...
    blocks: [
      {
        ...
        caption: 'Diffusion + logistic growth minus the containment (kill-switch) loss term.',
      },
      {
        tex: 'P_{\\text{spread}} = p_0\\,\\frac{R}{R + K_R}',
        caption: 'Per-step colonisation probability, limited by local resource availability.',
      },
    ],
  },
  aeolian: {
    ...
    blocks: [
      ...
      },
      {
        ...
        caption: 'Cohesion-enhanced threshold — the crust adds an adhesive term γ/(ρₐd) (Eq. 8).',
      },
      {
    ...
    blocks: [
      {
        ...
        caption: 'Lab-derived cohesion feeds the same threshold equation as the aeolian module.',
      },
    ],
  },
  grainsize: {
    ...
    blocks: [
      {
        ...
        caption: 'CaCO₃ cementation vs grain diameter: a log-Gaussian sweet spot × a fine-side permeability (bacterial-penetration) roll-off.',
      },
      {
        tex: 'C(d) = 1 - \\prod_{p}\\big(1 - e_p(d)\\big)',
        caption: 'A grain of size d is held if AT LEAST ONE active binder covers it (probabilistic union of per-prong effectiveness eₚ).',
      },
      {
        tex: 'f_{\\text{bound}} = \\int C(d)\\,\\phi(\\ln d)\\,\\mathrm{d}\\ln d,\\qquad \\phi \\sim \\mathrm{LogNormal}(D_{50},\\sigma_g)',
        caption: 'Effective bound mass fraction: coverage weighted by the site grain-size distribution (UAE dune sand, D₅₀ ≈ 200 µm).',
      },
    ],
  },
  composite: {
    ...
    blocks: [
      {
        ...
        caption: 'Competitive Langmuir Ca²⁺ partition: free calcium c_f solves the shared mass balance; each prong keeps φ_Ca of its standalone binding (the high-affinity calcite sink wins). Plus co-expression burden β for γ-PGA & CA.',
      },
      {
        tex: '\\tilde\\gamma_i = \\gamma_i\\,\\phi_{\\mathrm{Ca}}\\,\\phi_{\\text{burden},i}',
        caption: 'Each prong’s cohesion is knocked down by the interactions it takes part in, before combining.',
      },
      {
        tex: '\\gamma_{\\text{total}} = \\sum_i \\tilde\\gamma_i + \\sum_{i<j} \\eta_{ij}\\sqrt{\\tilde\\gamma_i \\tilde\\gamma_j}',
        caption: 'Composite cohesion: adjusted load-sharing plus the constructive physicochemical synergy η.',
      },
      {
        tex: 'r_{\\text{combined}} = 1 - \\prod_i (1 - r_i)',
        caption: 'Redundancy: the combination fails a scenario only if every prong fails it.',
      },
    ],
  },
  curing: {
    ...
    blocks: [
      {
        ...
        caption: 'Maturation: each prong approaches its mature cohesion on a per-binder time constant τ_p (alginate fast, MICP slow).',
      },
      {
        tex: '\\gamma_p(m) = \\gamma_p^{\\text{mature}}\\,2^{-m/H_p}',
        caption: 'Field weathering: cohesion decays with a per-binder half-life H_p (calcite most durable, alginate shortest).',
      },
      {
        ...
        caption: 'Re-application is due at m* — when the total surviving cohesion drops below the floor needed to withstand the design wind (inverse aeolian threshold).',
      },
    ],
  },
  economic: {
    ...
    blocks: [
      {
        ...
        caption: 'Total cost of a combination: per-prong capital plus per-hectare operating cost over area A.',
      },
      {
        tex: 'A_{\\text{break-even}} = \\frac{C^{\\text{capex}}}{C_{\\text{conv}}^{\\text{/ha}} - C^{\\text{opex/ha}}}',
        caption: 'Area at which the biological treatment undercuts the conventional chemical/concrete baseline.',
      },
    ],
  },
};
