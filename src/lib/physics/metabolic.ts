...
function derivatives(p: MetabolicParams, kDeg: number, m: number, e: number, pga: number): [number, number, number] {
  const dm = p.alpha_m * 1.0 - p.beta_m * m;
  const de = p.alpha_e * m - p.beta_e * e;
  const dp = (p.k_cat * e * p.s_precursor) / (p.k_m + p.s_precursor) - kDeg * pga;
  return [dm, de, dp];
}

/** Integrate the system with classic 4th-order Runge–Kutta. */
export function simulateMetabolicODE(p: MetabolicParams, opts: MetabolicSolveOptions = {}): SimulationStep[] {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const [dm2, de2, dp2] = derivatives(p, kDeg, m + 0.5 * dt * dm1, e + 0.5 * dt * de1, pga + 0.5 * dt * dp1);
    const [dm3, de3, dp3] = derivatives(p, kDeg, m + 0.5 * dt * dm2, e + 0.5 * dt * de2, pga + 0.5 * dt * dp2);
    const [dm4, de4, dp4] = derivatives(p, kDeg, m + dt * dm3, e + dt * de3, pga + dt * dp3);
...
export function finalYield(p: MetabolicParams, opts?: MetabolicSolveOptions): number {
 ...
 */
export function steadyState(p: MetabolicParams): { mRNA: number; enzyme: number; pga: number | null } {
 ...
 */
export function calibrateKcat(p: MetabolicParams, targetYield: number, opts?: MetabolicSolveOptions): number {
 ...
 */
export function yieldSensitivity(p: MetabolicParams, opts?: MetabolicSolveOptions): Record<string, number> {
  const base = finalYield(p, opts);
  const keys: (keyof MetabolicParams)[] = ['alpha_m', 'beta_m', 'alpha_e', 'beta_e', 'k_cat', 's_precursor', 'k_m'];
  const out: Record<string, number> = {};
  if (base <= 0) { keys.forEach(k => (out[k] = 0)); return out; }
  for (const k of keys) {
    const theta = p[k] as number;
    if (theta === 0) { out[k] = 0; continue; }
 ...
 */
export const fbaPrecursorToConc = (vGlu: number): number => Math.max(0, vGlu) * cval(FBA_CALIB.fluxToConc);
